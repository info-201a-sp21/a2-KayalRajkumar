...
make_introduction <- function(name, age) {
  paste0("Hello, my name is", sep = "_", name, ", and I'm", sep = "_", age,
         sep = "_", "years old.")
}
...
has_more_zs <- function(str1, str2) {
  instances_of_z_str1 <- str_count(str1, "z") + str_count(str1, "Z")
  instances_of_z_str2 <- str_count(str2, "z") + str_count(str2, "Z")
  if (instances_of_z_str1 == 0 & instances_of_z_str2 == 0) {
    sentence <- paste("Neither string contains the letter z.")
  } else if (instances_of_z_str1 > instances_of_z_str2) {
    sentence <- paste("String 1 has more instances of zs than string 2.")
  } else if (instances_of_z_str2 > instances_of_z_str1) {
    sentence <- paste("String 2 has more instances of zs than string 1.")
  } else {
...
more_zs <- has_more_zs("I am tired.", "I am going to the park now.")
...
practice <- remove_digits(c("INFO 201", "PSYCH 318", "PSYCH 448"))

### Vectors ###################################################################

# Create a vector `movies` that contains the names of six movies you like
movies <- c("Up", "Tangeled", "Moana", "Cars", "Soul", "Mulan")
...
summarize_vector <- function(numbers_vector) {
  vector_mean <- mean(numbers_vector)
  vector_median <- median(numbers_vector)
  vector_length <- length(numbers_vector)
  list(mean = vector_mean, median = vector_median, length = vector_length)
}
...
values_one_through_hundred <- seq(1, 100)
summary_1_to_100 <- summarize_vector(values_one_through_hundred)
